/** Gauntlet 4 §54 — 200-seed teapot+tasting-challenge production stress corpus. */
...
describe('teapot + tasting challenge stress (200 seeds)', () => {
  it('every production level is structure- + solver-valid and uses the bowl', async () => {
    for (let s = 0; s < 200; s++) {
      const seed = `tasting-stress-teapot:${s}`;
