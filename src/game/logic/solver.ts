...
export function solvePuzzle(cups: TeaId[][], opts: SolverOptions = {}): SolverResult {
  ...
  while (head < queue.length) {
    ...
    // (Homogeneous stack -> empty only permutes identical cups.)
    const moves = listLegalMoves(node.cups, true);

    for (const m of moves) {
      const res = applyPour(node.cups, m.from, m.to);
      if (!res) continue;
      const key = canonicalKey(res.cups);
 ...
 */
export function applySolution(cups: TeaId[][], solution: SolverMove[]): TeaId[][] | null {
  ...
  for (const step of solution) {
    ...
    const res = applyPour(board, step.from, step.to);
    if (!res) return null;
    void isHomogeneous;
    board = res.cups;
  }
