...
export class Cup {
  ...
  canPourInto(target: Cup): boolean {
    ...
    // (Two-cup board is enough because rules only inspect the pair.)
    return canPourBetween([this.layers, target.layers], 0, 1);
  }

  pourInto(target: Cup): { transferred: number; layer: TeaId } | null {
    const res = applyPour([this.layers, target.layers], 0, 1);
    if (!res) return null;
...
export class TeaSortLogic {
  ...
  toState(): { cups: TeaId[][]; hiddenCounts: number[] } {
    return {
  ...
  canMakeMove(fromIdx: number, toIdx: number): boolean {
    ...
    return canPourBetween(board, fromIdx, toIdx);
  }
  ...
  isWon(): boolean {
    return isWonState(this.cups.map((c) => c.layers));
  }

  isDeadlocked(): boolean {
    return isDeadlockedState(this.cups.map((c) => c.layers));
  }
}
